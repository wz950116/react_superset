...
function getMaxLabelSize(container, axisClass) {
  ...
  const labelDimensions = labelEls.map(
    i => labelEls[i].getComputedTextLength());
...
function nvd3Vis(slice, payload) {
  ...
  const drawGraph = function () {
    switch (vizType) {
      case 'funnel':
        // console.log('This is funnel show');
        break;
      ...
      case 'compare_line':
        ...
        // chart.interactiveLayer.tooltip.headerFormatter(
        //   function () {
        //     return 'headerFormatter';
        //   });
        if (vizType === 'compare_line') {
          chart.interactiveLayer.tooltip.contentGenerator(function (obj) {
            ...
            for (const i in data.series) {
              ...
              const currDay = moment(s.data.x).format('YYYY-MM-DD');
              const dateRange = s.key.split('~');
              if (dateRange.length > 1) {
                const sday = moment(dateRange[0]);
                const eday = moment(dateRange[1]);
                let tmpDay = sday;
                const dayRange = [];
                while (tmpDay <= eday) {
                  dayRange.push(tmpDay.format('YYYY-MM-DD'));
                  tmpDay = tmpDay.add(1, 'day');
                }
                data.series[i].dayRange = dayRange;
                const dindex = dayRange.indexOf(currDay);
                if (dindex > -1) {
                  // currDay = dayRange[dindex];
                  dayIndex = dindex;
                }
              }
            }
            for (const i in data.series) {
              ...
              // header
              tip += `<tr><td colspan="3"><strong class="x-value">${s.dayRange[dayIndex]}</strong></td></tr>`;
              tip += (`<tr><td class="legend-color-guide"><div style="background-color: ${s.color};"></div></td>` +
                `<td>${s.key}</td><td>${s.value}</td></tr>`);
            }
            tip += '</table>';
            return tip;
          });
    ...
    if ((vizType === 'line' || vizType === 'area' || vizType ===
        'compare_line') && fd.rich_tooltip) {
    ...
    if (vizType === 'dual_line') {
      ...
      customizeToolTip(chart, xAxisFormatter,
        [yAxisFormatter1, yAxisFormatter2]);
      chart.showLegend(width > BREAKPOINTS.small);
    }
    const data = payload.data;

    if (['line', 'area'].indexOf(vizType) > -1 && payload.errdata) {
      for (let i = 0; i < payload.errdata.length; i++) {
        for (let j = 0; j < payload.errdata[i].length; j++) {
          data[i].values[payload.errdata[i][j]].error_data = true;
        }
      }
    }
    svg

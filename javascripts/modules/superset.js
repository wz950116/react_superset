...
const px = function () {
  ...
  const Slice = function (data, datasource, controller) {
    ...
    slice = {
      ...
      render(force) {
        ...
        $.getJSON(this.jsonEndpoint(), (queryResponse) => {
          try {
            ...
            resp.csv_url = this.endpoint('csv');
            this.csv_url = resp.csv_url;
            vizMap[formData.viz_type](this, queryResponse);
            this.done(queryResponse);
          } catch (e) {
            this.error('An error occurred while rendering the visualization: ' +
              e);
          }
        }).fail((err) => {
      ...
      addFilter(col, vals, merge = true, refresh = true, addToUrl = true) {
        controller.addFilter(sliceId, col, vals, merge, refresh, addToUrl);
      },

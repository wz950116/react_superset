...
export default class SelectControl extends React.PureComponent {
  ...
  componentDidMount() {
    if (this.props.name === 'order_by_cols') {
      // 监听 msg 事件
      eventProxy.on('orderBy', (msg) => {
        let options = this.getOptions(this.props);
        const orderByArr = [];
        for (let i = 0; i < msg.length; i++) {
          const o = msg[i];
          const oAsc = { label: o.label + ' [asc]', value: '["' + o.value + '", true]' };
          const oDesc = { label: o.label + ' [desc]', value: '["' + o.value + '", false]' };
          orderByArr.push(oAsc);
          orderByArr.push(oDesc);
        }
        options = options.concat(orderByArr);
  ...
  onChange(opt) {
    ...
    if (this.props.name === 'metrics') {
      setTimeout(() => {
        // 发布 msg 事件
        eventProxy.trigger('orderBy', opt);
      }, 1000);
    }
  ...
  getOptions(props) {
    ...
    if (this.props.name === 'metrics') {
      setTimeout(() => {
        // 发布 msg 事件
        eventProxy.trigger('orderBy', options);
      }, 1000);
    }
    return options;
  }
  ...
  render() {
    ...
    //  Tab, comma or Enter will trigger a new option created for FreeFormSelect
    const selectWrap = this.props.freeForm ? (<Creatable {...selectProps} />) : (
      <Select {...selectProps} />);
    return (
      <div >
        {selectWrap}

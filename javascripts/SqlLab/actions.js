...
export function syncTime(query, results) {
  return { type: SYNC_TIME, query, results };
}
...
export function fetchQueryResults(query1) {
  return function (dispatch) {
    if (query1.state === 'stopped') {
      dispatch(stopQuery(query1));
      return;
    }
    dispatch(requestQueryResults(query1));
    const query = query1;
    if (!query.page) {
      query.page = 1;
    }
    const sqlJsonUrl = `/superset/results/${query.resultsKey}/?page=` + query.page + '&id=' + query.id;
    $.ajax({
      type: 'GET',
      dataType: 'json',
      url: sqlJsonUrl,
...
export function runQuery(query) {
  return function (dispatch) {
    ...
    $.ajax({
      ...
      success(results) {
        if (query.runAsync) {
          dispatch(syncTime(query, results));
        } else {
          dispatch(querySuccess(query, results));
        }
      },
